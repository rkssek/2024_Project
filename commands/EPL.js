...
async function getEPLRankings() {
    try {
        const { data } = await axios.get('https://www.espn.com/soccer/standings/_/league/eng.1');
...
module.exports = {
    name: 'epl',
    description: '현재 EPL 상위 10위 팀을 보여줍니다.',
    async run(client, message, args) {
        const rankings = await getEPLRankings();
        if (rankings.length > 0) {
            message.channel.send(`현재 EPL 순위:\n${rankings.join('\n')}`);
        } else {
            message.channel.send('EPL 순위를 가져오는 데 실패했습니다.');
        }
    },
};
